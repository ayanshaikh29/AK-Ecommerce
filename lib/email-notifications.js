...
const FROM_EMAIL = process.env.EMAIL_FROM || 'AK Enterprises <sales@akcorporateworld.com>'
const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.akcorporateworld.com'
...
export async function sendOrderConfirmedEmails(orderIdOrObj, options = {}) {
  ...
  // Sandbox override: if sender is onboarding@resend.dev, redirect all emails to buildwithayan29@gmail.com
  const isSandbox = FROM_EMAIL.includes('onboarding@resend.dev')

  if (customerEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : customerEmail,
      subject: (isSandbox ? '[TEST - Customer] ' : '') + `Order Confirmed — #${orderNumber} | AK Enterprises`,
  ...
  if (vendorEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : vendorEmail,
      subject: (isSandbox ? '[TEST - Zonal Admin] ' : '') + `Order Ready for Fulfillment — #${orderNumber}`,
  ...
  if (ownerEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : ownerEmail,
      subject: (isSandbox ? '[TEST - Owner] ' : '') + `Order Confirmed — #${orderNumber} | ${formatCurrency(fullOrder.total)}`,
...
function buildCustomerDeliveredEmail(order, items, customerName, addressObj) {
  const addressStr = [
    addressObj.line1 || '',
    addressObj.line2 || '',
    `${addressObj.city || ''}, ${addressObj.state || ''} - ${addressObj.pincode || ''}`,
  ...
  return emailWrapper(`
    <h2 style="color:#1a1a2e;font-size:20px;margin:0 0 4px">Order Delivered! 🎉</h2>
    <p style="color:#333;font-size:15px;line-height:1.5;margin:0 0 16px">Dear ${customerName},</p>
    <p style="color:#333;font-size:15px;line-height:1.5;margin:0 0 24px">Your order <strong>#${order.order_number}</strong> has been successfully delivered to your location.</p>

    <table style="width:100%;background:#f9f9f9;border-radius:8px;padding:16px;margin-bottom:20px">
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Order Number</td><td style="font-size:13px;font-weight:bold;color:#1a1a2e;text-align:right;padding:4px 0">#${order.order_number}</td></tr>
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Total Amount</td><td style="font-size:13px;font-weight:bold;color:#1a1a2e;text-align:right;padding:4px 0">${formatCurrency(order.total)}</td></tr>
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Delivery Address</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${addressObj.city || ''}</td></tr>
    </table>

    <h3 style="color:#1a1a2e;font-size:14px;margin:0 0 8px">Items Delivered</h3>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
      <thead><tr style="background:#f0f0f0">
        <th style="padding:8px 12px;font-size:11px;text-transform:uppercase;color:#666;text-align:left;font-weight:600">Product Name</th>
        <th style="padding:8px 12px;font-size:11px;text-transform:uppercase;color:#666;text-align:center;font-weight:600">Quantity</th>
    ...
    <div style="text-align:center;margin-top:24px">
      <a href="${BASE_URL}/orders/${order.id}" style="display:inline-block;background:#1a1a2e;color:#f5c518;text-decoration:none;padding:12px 32px;border-radius:24px;font-size:13px;font-weight:bold">View Order Summary</a>
    </div>
  `)
}

function buildZonalAdminDeliveredEmail(order, items, customerName, customerCompany, addressObj) {
  return emailWrapper(`
    <h2 style="color:#1a1a2e;font-size:20px;margin:0 0 4px">Order Delivered Successfully ✅</h2>
    <p style="color:#333;font-size:14px;line-height:1.5;margin:0 0 24px">Great job! Order <strong>#${order.order_number}</strong> has been marked as delivered to <strong>${customerName}</strong>.</p>

    <table style="width:100%;background:#f9f9f9;border-radius:8px;padding:16px;margin-bottom:20px">
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Customer Name</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${customerName}</td></tr>
      ${customerCompany ? `<tr><td style="font-size:13px;color:#666;padding:4px 0">Company Name</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${customerCompany}</td></tr>` : ''}
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Total Value</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${formatCurrency(order.total)}</td></tr>
    </table>

    <div style="text-align:center;margin-top:24px">
      <a href="${BASE_URL}/vendor" style="display:inline-block;background:#1a1a2e;color:#f5c518;text-decoration:none;padding:12px 32px;border-radius:24px;font-size:13px;font-weight:bold">Go to Portal</a>
    </div>
  `)
}

function buildOwnerDeliveredEmail(order, items, customerName, customerCompany, vendorName, deliveredTime) {
  return emailWrapper(`
    <h2 style="color:#1a1a2e;font-size:20px;margin:0 0 4px">Order Delivered 📦</h2>
    <p style="color:#333;font-size:14px;line-height:1.5;margin:0 0 24px">Order <strong>#${order.order_number}</strong> has been delivered successfully.</p>

    <table style="width:100%;background:#f9f9f9;border-radius:8px;padding:16px;margin-bottom:20px">
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Customer Name</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${customerName}</td></tr>
      ${customerCompany ? `<tr><td style="font-size:13px;color:#666;padding:4px 0">Company Name</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${customerCompany}</td></tr>` : ''}
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Zonal Admin (Vendor)</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${vendorName || 'Not assigned'}</td></tr>
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Total Amount</td><td style="font-size:13px;font-weight:bold;color:#1a1a2e;text-align:right;padding:4px 0">${formatCurrency(order.total)}</td></tr>
      <tr><td style="font-size:13px;color:#666;padding:4px 0">Delivered Timestamp</td><td style="font-size:13px;color:#333;text-align:right;padding:4px 0">${formatDate(deliveredTime)}</td></tr>
    </table>
...
export async function sendOrderDeliveredEmails(orderIdOrObj, options = {}) {
  const supabase = options.supabaseClient || db()
  const results = { sent: 0, errors: [] }
  ...
  if (customerEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : customerEmail,
      subject: (isSandbox ? '[TEST - Customer] ' : '') + `Order Delivered 🎉 | #${orderNumber} | AK Enterprises`,
      html: buildCustomerDeliveredEmail(fullOrder, items, customerName, addressObj),
      recipientType: 'customer'
    })
  }
  ...
  if (vendorEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : vendorEmail,
      subject: (isSandbox ? '[TEST - Zonal Admin] ' : '') + `Order Delivered Successfully — #${orderNumber}`,
      html: buildZonalAdminDeliveredEmail(fullOrder, items, customerName, customerCompany, addressObj),
      recipientType: 'zonal_admin'
    })
  }
  ...
  if (ownerEmail) {
    emails.push({
      to: isSandbox ? 'buildwithayan29@gmail.com' : ownerEmail,
      subject: (isSandbox ? '[TEST - Owner] ' : '') + `Order Delivered — #${orderNumber} | ${formatCurrency(fullOrder.total)}`,
      html: buildOwnerDeliveredEmail(fullOrder, items, customerName, customerCompany, vendorName, fullOrder.updated_at || new Date().toISOString()),
      recipientType: 'owner'
    })
  }

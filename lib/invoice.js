...
export const downloadInvoice = (order) => {
  ...
  const sameState = gstBreakdown?.sameState ?? true
  const supplierState = gstBreakdown?.supplierState || 'Maharashtra'
  const customerState = order.address?.state || gstBreakdown?.customerState || ''
  ...
  // Totals from gst_breakdown or computed
  const gbd = gstBreakdown || {}
  const totalTaxable = gbd.totalTaxable ?? (order.items || []).reduce((s, it) => {
  ...
  const totalCGST = gbd.totalCGST ?? (sameState ? (order.total - (order.shipping_fee || 0) - totalTaxable) / 2 : 0)
  const totalSGST = gbd.totalSGST ?? (sameState ? (order.total - (order.shipping_fee || 0) - totalTaxable) / 2 : 0)
  const totalIGST = gbd.totalIGST ?? (!sameState ? (order.total - (order.shipping_fee || 0) - totalTaxable) : 0)

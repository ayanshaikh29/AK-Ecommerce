...
export async function getMinOrderQuantity() {
  ...
  try {
    const { data } = await supabase.from('settings').select('min_order_quantity').eq('id', 'main').maybeSingle()
    if (data && data.min_order_quantity !== undefined && data.min_order_quantity !== null) {
      return Number(data.min_order_quantity) || 6000
    }
  } catch (e) {}
  return 6000
}
...
export async function setMinOrderQuantity(moq) {
  const supabase = getDb()
  const val = Math.max(1, Number(moq) || 6000)
  await supabase.from('settings').upsert({ id: 'main', min_order_quantity: val, updated_at: new Date().toISOString() })
  return val
}
...
export async function getVendorByUserId(userId) {
  ...
  return list.find(v => v.user_id === userId) || null
}

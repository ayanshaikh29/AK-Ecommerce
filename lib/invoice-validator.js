export function validateInvoiceData(order, settings, isChallan = false) {
  ...
  if (!addr.full_name && !profile.business_name && !addr.business_name) {
    return { valid: false, error: 'Cannot generate document: missing customer name (business name or full name)' }
  }
  if (!addr.phone) {

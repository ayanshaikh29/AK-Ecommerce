...
export async function generateInvoicePDF(order, settings) {
  ...
  if (customerGst) {
    doc.setFont('Helvetica', 'bold')
    doc.setFontSize(8)
    doc.text(`GSTIN: ${customerGst}`, ML + 2, y); y += 3.5
  }
  ...
  parsedItems.forEach((it, idx) => {
    ...
    doc.setFont('Helvetica', 'bold')
    doc.text(formatINR(it.taxableValue), cAmt + 18, tableY + 3.5, { align: 'right' })
    doc.setFont('Helvetica', 'normal')
  ...
  doc.text(`Rs. ${formatINR(order.total)}`, cAmt + 18, tableY + 4.5, { align: 'right' })
  tableY += 6

...
export async function generateChallanPDF(order, settings) {
  ...
  if (line1) consigneeAddrLines.push(...doc.splitTextToSize(line1, 80))
  if (line2) consigneeAddrLines.push(...doc.splitTextToSize(line2, 80))
  consigneeAddrLines.push(...doc.splitTextToSize(`${city}, ${state} - ${pincode}`, 80))
  consigneeAddrLines.push(`Phone: ${customerPhone}`)
  if (customerGST) {
  ...
  orderItemsList.forEach(it => {
    ...
    const requiredHeight = Math.max(rowHeight, 6) + 12

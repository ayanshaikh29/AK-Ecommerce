...
export async function generateChallanPDF(order, settings) {
  ...
  // Retrieve stamp (either custom URL or local fallback)
  let stampData = null
  const customUrl = settings.company_stamp_url
  if (customUrl && (customUrl.startsWith('http://') || customUrl.startsWith('https://'))) {
    try {
      const imgRes = await fetch(customUrl)
      if (imgRes.ok) {
        const arrayBuffer = await imgRes.arrayBuffer()
        const buffer = Buffer.from(arrayBuffer)
        stampData = {
          base64: buffer.toString('base64'),
          format: customUrl.toLowerCase().endsWith('.png') ? 'PNG' : 'JPEG'
        }
      }
    } catch (e) {
      console.error('Failed to fetch custom stamp:', e.message)
    }
  }
  ...
  if (!stampData) {
    try {
      ...
      console.error('Failed to read local stamp-signature.png:', e.message)
    }
  }

  if (stampData) {
    try {
      // Centered at X=152, width=42, so X = 152 - (42/2) = 131
      // Stamp image is pre-cropped with correct aspect ratio (~2.52:1), sits between
      // the "for AK Enterprises" heading (tableY+4) and the signature line (sigY = tableY+26),
      // lightly crossing the line without touching "Authorised Signatory" (sigY+3)
      doc.addImage(
        `data:image/${stampData.format.toLowerCase()};base64,${stampData.base64}`,
        stampData.format,
        132,
        tableY + 8,
        40,
      ...
      console.error('Failed to add stamp image to challan PDF:', e.message)
    }
  }

...
async function zohoGet(path) {
  ...
  return httpsGet(ZOHO_API, `/books/v3${path}?organization_id=${ZOHO_ORG_ID}`, {
...
async function zohoPost(path, body) {
  ...
  return httpsPost(ZOHO_API, `/books/v3${path}?organization_id=${ZOHO_ORG_ID}`, bodyStr, {
    Authorization: `Zoho-oauthtoken ${token}`,
...
async function zohoGetPdf(path) {
  const token = await getZohoAccessToken()
  return httpsGetBuffer(ZOHO_API, `/books/v3${path}?organization_id=${ZOHO_ORG_ID}&accept=pdf`, {
    Authorization: `Zoho-oauthtoken ${token}`,
...
export async function syncZohoContact(customer) {
  ...
  try {
    ...
    const payload = {
      ...
      billing_address: {
        address: customer.shipping_address?.address_line1 || '',
        city: customer.shipping_address?.city || '',
        state: customer.shipping_address?.state || '',
        zip: customer.shipping_address?.postal_code || '',
        country: 'India'
      }
    }
...
export async function createZohoInvoice({ order, contactId }) {
  ...
  const customerState = (order.shipping_address?.state || '').trim().toUpperCase()
  const isInterState = customerState !== 'MAHARASHTRA' && customerState !== 'MH'

  const lineItems = (order.items || []).map(item => {
    const rate = item.price_snapshot || 0
    const gstRate = item.gst_rate || 18 // default 18% GST
    const itemObj = {
  ...
  try {
    ...
    if (res?.invoice?.invoice_id) return res.invoice.invoice_id
